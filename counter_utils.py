...
def start_simple_counter(video, debug, output):
    ...
    def TrackMove(rec, pfocus):
        nonlocal pointMove, firstTrack, statusMove
        # find track
    ...
    def setInitialVariable(w, h):
        nonlocal W, H, frameSize, areaFrame, MinContourArea, MaxContourArea, OffsetX, OffsetY, sizeLong, padding, margin
    ...
    while True:
        ...
        for c in cnts:
            ...
            if abs(firstCenter[1] - CoorExitLine1[1]) > margin and abs(ObjectCentroid[1] - CoorExitLine1[1]) > margin:
                if not CheckLineCrossing(firstCenter, CoorExitLine1, CoorExitLine2) and CheckLineCrossing(
                        ObjectCentroid,
                        CoorExitLine1,
                        CoorExitLine2):
                    countPeople -= 1
                    firstTrack[indexTrack] = rec
                elif not CheckLineCrossing(ObjectCentroid, CoorExitLine1, CoorExitLine2) and CheckLineCrossing(
                        firstCenter,
                        CoorExitLine1,
                        CoorExitLine2):
                    countPeople += 1
